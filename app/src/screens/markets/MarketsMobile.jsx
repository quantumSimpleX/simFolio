...
import { StatusBar, Eyebrow, MktStatus } from '../../components/Primitives'
...
export default function MarketsMobile() {
  ...
  return (
    <div style={{ width:390, minHeight:844, background:C.paper, display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ padding:'0 24px 14px', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:`1px solid ${C.ink100}`, flexShrink:0 }}>
        <div style={{ fontFamily:SANS, fontWeight:700, fontSize:22, color:C.ink900 }}>Markets</div>
        <MktStatus open={marketOpen}/>
      </div>

      {/* Search */}
      <div style={{ padding:'12px 24px', borderBottom:`1px solid ${C.ink100}`, flexShrink:0, position:'relative' }}>
        <div style={{ height:44, background:C.white, border:`1px solid ${search ? C.ame400 : C.ink200}`, borderRadius:4, display:'flex', alignItems:'center', padding:'0 14px', gap:10 }}>
          ...
          <input
            ...
            style={{ flex:1, border:'none', outline:'none', fontFamily:SANS, fontSize:15, color:C.ink900, background:'transparent' }}
          />
          {search && <div onClick={() => setSearch('')} style={{ fontFamily:SANS, fontSize:18, color:C.ink300, cursor:'pointer' }}>×</div>}
        </div>

        {showDropdown && (
          <div style={{ position:'absolute', left:24, right:24, top:56, background:C.white, border:`1px solid ${C.ink100}`, borderRadius:4, boxShadow:'0 4px 12px rgba(0,0,0,0.08)', zIndex:10, overflow:'hidden' }}>
            {searchResults.map(r => (
              <SearchResultRow
                key={r.symbol}
                r={r}
            ...
            {/* Footer actions: only relevant when treating input as a direct ticker */}
      ...
      <div style={{ flex:1, padding:'16px 24px', display:'flex', flexDirection:'column', gap:18, overflowY:'auto' }}>
        {/* Indices */}
        <div>
            ...
            {INDEX_SYMBOLS.map(sym => {
              ...
              return (
                <div key={sym} style={{ flex:1, background:C.white, border:`1px solid ${C.ink100}`, borderRadius:8, padding:'12px 14px' }}>
                  ...
                  <div style={{ fontFamily:SANS, fontWeight:700, fontSize:18, color:C.ink900, marginTop:4 }}>
