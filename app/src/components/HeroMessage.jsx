    ...
    <div style={{ display:'flex', gap:10 }}>
      ...
      <div style={{ flex:1 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
          <div style={{ fontFamily:SANS, fontSize:17, fontWeight:600, color:C.ink700 }}>{h.name}</div>
          {time && <div style={{ fontFamily:SANS, fontSize:13, color:C.ink300 }}>{time}</div>}
          {isNew && <div style={{ width:6, height:6, borderRadius:'50%', background:C.ame400, marginLeft:'auto' }}/>}
        </div>
        <div style={{ fontFamily:SANS, fontSize:18, color:C.ink600, lineHeight:1.55, fontStyle:'italic' }}>{text}</div>
      </div>
    </div>
  );
}
...
export function UserMessage({ text }) {
  return (
    <div style={{ display:'flex', justifyContent:'flex-end' }}>
      <div style={{ maxWidth:'80%', background:C.ink900, color:C.white, padding:'10px 14px', borderRadius:'8px 8px 2px 8px', fontFamily:SANS, fontSize:18, lineHeight:1.5 }}>{text}</div>
    </div>
  );
}
...
export function SageMsg({ text, compact=false }) {
  ...
  return (
    <div style={{ display:'flex', gap:10 }}>
      ...
      <div style={{ flex:1 }}>
        <div style={{ fontFamily:SANS, fontSize:13, fontWeight:600, color:C.ink400, letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:6 }}>Sage</div>
        <div style={{ fontFamily:SANS, fontSize:compact?15:18, color:C.ink600, lineHeight:1.55 }}>{text}</div>
      </div>
    </div>
  );
}
