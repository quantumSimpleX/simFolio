...
export function BadgeGlyph({ type='diamond', size=36, color=C.aqua400, earned=false }) {
  const op = earned ? 1 : 0.28;
  const s = size;
  const h = s/2;
  if (type === 'diamond') {
    return (
      <svg width={s} height={s} style={{ opacity:op }}>
...
export function MedalGlyph({ size=36, earned=false }) {
...
export function TrophyGlyph({ size=44, earned=false }) {
  const op = earned ? 1 : 0.3;
  const s = size, h = s/2;
...
export function BadgeGlyphForIndex({ index, size=36, earned=false }) {
  return <BadgeGlyph type={GLYPH_TYPES[index%GLYPH_TYPES.length]} size={size} color={GLYPH_COLORS[index%GLYPH_COLORS.length]} earned={earned}/>;
}
